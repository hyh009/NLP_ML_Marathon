...
**2. 針對不存在於字典的字詞：**<br>
   ...
   **狀態轉移矩陣：** 從1個狀態轉移到下一個狀態時的機率。<br>
   ...
   ★ **隱馬可夫模型(HMM)：主要可以用來解決三種經典的問題↓**<br>
   
   **◎預測(filter) :** 已知模型參數和某一特定輸出序列，求最後時刻各個隱含狀態的機率分布，通常使用前向演算法解決。<br>
   **◎平滑(smoothing) :** 已知模型參數和某一特定輸出序列，求中間時刻各個隱含狀態的機率分布，通常使用前向-後向演算法解決。<br>
   **◎解碼(most likely explanation) :**  已知模型參數，尋找最可能的能產生某一特定輸出序列的隱含狀態的序列，通常使用Viterbi演算法解決。<br>
   
   Pos Tagging 可透過**解碼**方式解決<br>
   **當無法直接觀察到的隱藏狀態時，利用可觀測到的觀察狀態，來推測隱藏狀態的機率，(透過Viterbi演算法找出最大機率路徑)。**
...
**2. Naïve Bayes [作業-Naive_Bayes實作](https://github.com/hyh009/NLP_ML_Marathon/blob/master/Day23_Naive_Bayes%E5%AF%A6%E4%BD%9C%E4%BD%9C%E6%A5%AD.ipynb)**<br>
  ...
  ★ 為什麼naïve？   →    假設所有的輸入特徵都是**彼此獨立** → **順序也無關**。<br>
  ★ scikit-learn API比較↓
